...
class ASTGCN_block(nn.Module):
    ...
    def __init__(self, DEVICE, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials, num_of_vertices, num_of_timesteps):
        ...
        # nn.Conv2d计算公式
        # H_out = floor((H_in + 2 * padding[0] - (kernel_size[0] - 1) + 1) / stride[0]))
        # W_out = floor((W_in + 2 * padding[1] - (kernel_size[1] - 1) + 1) / stride[1]))
    ...
    def forward(self, x):
        ...
        time_conv_output = self.time_conv(spatial_gcn.permute(0, 2, 1, 3))  # (b,N,F,T)->(b,F,N,T) 用(1,3)的卷积核去做->(b,F,N,T),即(32,64,170,12)
...
def make_model(DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, adj_mx, num_for_predict, len_input, num_of_vertices):
    ...
    # Chebyshev polynomial as convolution kernel
    L_tilde = scaled_Laplacian(adj_mx)
    cheb_polynomials = [torch.from_numpy(i).type(torch.FloatTensor).to(DEVICE) for i in cheb_polynomial(L_tilde, K)]
    # cheb_polynomials is a matrix (N,N)
