...
def search_data(sequence_length, num_of_depend, label_start_idx,
    ...
    x_idx = []
    # choosing the time series(1week/ 1day/ 1hour) as a train dataset
    for i in range(1, num_of_depend + 1):
        # the starting index position
        start_idx = label_start_idx - points_per_hour * units * i
        # the ending index position
        end_idx = start_idx + num_for_predict
        if start_idx >= 0:
            x_idx.append((start_idx, end_idx))
        else:
            return None
    # judge whether the element in x_idx=1 is equal to num_of_depend=1(num_of_predict)
...
def read_and_generate_dataset(graph_signal_matrix_filename,
    ...
    for idx in range(data_seq.shape[0]):
        ...
        # hour_sample.shape: (1, N, F, T) = (1, 170, 3, 12), N(num of tensors), F(Features), T(Time stamp)
        # np.expand_dims: (12, 370, 3) to (1, 12, 370, 3); transpose: (1, 12, 370, 3) to (1, 170, 3, 12)
        if num_of_hours > 0:
    ...
    # dividing the train dataset(60%) validation dataset(20%) and test dataset(20%)
    split_line1 = int(len(all_samples) * 0.6)
    split_line2 = int(len(all_samples) * 0.8)

    # {list:3}
    # [(B,N,F,Tw),(B,N,F,Td),(B,N,F,Th),(B,N,Tpre),(B,1)]
    # [X:(10699, 170, 3, 12), Y:(10699, 170, 12), index:(10699, 1)]
